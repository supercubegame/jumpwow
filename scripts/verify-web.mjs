...
// 改了那边的配色而这里不同步，platformInk 会数到 0,它会红，不会假绿。
const PLAT_PALETTE = ['#7ee0a4', '#3f9a6a',    // NORMAL
                      '#7cc8f5', '#3a7fae',    // MOVING
                      '#e8bd7c', '#a37438',    // FRAGILE
                      '#f59ad4', '#a94f8c',    // SPRING
                      '#ffd76b'];              // 角色身体
// 游戏画面上「内容色」至少要占这么多像素。只抓「平台整块没画出来」，不是
// 排版基准。首轮实测值见报告，之后按实测的三分之一收紧。
const PLAT_INK_MIN = Number(process.env.PLAT_INK_MIN || 0.001);
...
try{
   ...
   * 内容像素 5.38%、机器人在浏览器里跳到 90，渲染明明是好的。它从写下那天
   * 起就是一次抛硬币，只是一直抛正面。
   *
